...
import { isLinked } from '../lib/accountTypes'
...
export function AccountCard({ account, balance, delta, lastSnapshotAt, onTap, onEdit }: Props) {
  ...
  return (
    <div className="relative">
      {/* Main tap target — a real <button> with no interactive children */}
      <button
        type="button"
        onClick={onTap}
        className="wallet-card w-full text-left cursor-pointer block"
        style={{ background: `linear-gradient(135deg, ${gradient.from}, ${gradient.to})` }}
      >
        <div className="relative p-5 flex flex-col" style={{ minHeight: '148px' }}>
          {/* Top row */}
          <div className="flex items-start mb-auto">
            <div className={onEdit ? 'flex-1 min-w-0 pr-10' : 'flex-1 min-w-0'}>
              <p className="text-sm font-semibold text-white truncate leading-snug">{account.name}</p>
      ...
      {/* Edit button — outside the main button to avoid nested interactive elements */}
      {onEdit && (
        <button
          type="button"
          onClick={e => { e.stopPropagation(); onEdit() }}
          aria-label="Edit account"
          className="absolute top-3 right-3 p-2 min-w-[44px] min-h-[44px] flex items-center justify-center text-white/50 hover:text-white transition-colors rounded-lg z-10"
        >
